...
#include <dlfcn.h>
#cgo LDFLAGS: -ldl
void* my_initialize(void* f, const char* model_entry,
...
int my_process(void* f, void* model_buf, const void* input_data,
...
func main() {
	...
	model := C.my_initialize(initialize, (*C.char)(unsafe.Pointer(&modelEntry[0])), (*C.char)(unsafe.Pointer(&modelConfig[0])), &state)
	...
	state = C.my_process(process, model, unsafe.Pointer(&buffer[0]), size, &output, &outputSize)
